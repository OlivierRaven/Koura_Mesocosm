...
# RQ2 — Sex difference in structure preference at next_day
tab1_sex     <- table(df1_next_day$structure_type, df1_next_day$sex)
...
df3_next_day <- df3_next_day %>% mutate(structure_type = relevel(factor(structure_type), ref = "FlatStone"))
...
if (!file.exists(m_path3)) {
  m_exp3_bayes <- brm(
    structure_type ~ size_class + (1 | group_id/koura_id) + (1 | round),
    data = df3_next_day %>% filter(stage == "next_day"),
    family = categorical(),
...
if (!file.exists(m_path4)) {
  m_exp4_bayes <- brm(
    ...
    data = df4 %>% filter(stage == "next_day"),
    family = categorical(),
    cores = 4, iter = 4000, open_progress = FALSE,
    control = list(adapt_delta = 0.99, max_treedepth = 12))
