...
# RQ1: Is there a preference for a specific reef type at L1?
# RQ2: Does reef type preference differ between sexes at L1?
# RQ3: Did reef type use shift directionally between L0 and L1?
...
xmulti1      <- XNomial::xmulti(
  obs  = as.numeric(tab1_L1_nz),
  expr = rep(sum(tab1_L1_nz) / length(tab1_L1_nz), length(tab1_L1_nz)))
...
# Post-hoc pairwise G-tests between sex
pgtest_female <- pairwise_gtest(table(df1_L1$reef_type[df1_L1$sex == "F"]))
pgtest_male   <- pairwise_gtest(table(df1_L1$reef_type[df1_L1$sex == "M"]))

# RQ3 — Directional shift between L0 and L1
df1_wide     <- df1 %>%
...
gtest_to_cld <- function(pgtest_result) {
  # Create named vector of adjusted p-values with "A:B" format keys
  pvals <- setNames(
    pgtest_result$p_adj,
...
cld_female <- data.frame(
  stage      = "L1",
  panel      = "Female",
  reef_type  = names(gtest_to_cld(pgtest_female)),
  cld_letter = unname(gtest_to_cld(pgtest_female))
)

cld_male <- data.frame(
  stage      = "L1",
  panel      = "Male",
  reef_type  = names(gtest_to_cld(pgtest_male)),
  cld_letter = unname(gtest_to_cld(pgtest_male))
)

cld_all <- bind_rows(cld_combined, cld_female, cld_male)

exp1_reef_letters <- exp1_reef_all %>%
  relevel_reef() %>%
  filter(stage == "L1") %>%
  left_join(cld_all, by = c("stage", "panel", "reef_type"))
...
Exp1_reef_p <- exp1_reef_all %>%
  ...
  geom_text(
    data     = exp1_reef_letters,
    aes(y = prop + 0.04, label = cld_letter),
    size     = 3.5,
    fontface = "bold"
  ) +
  facet_grid(panel ~ stage,
             labeller = labeller(stage = lab_stage, panel = lab_panel1)) +
...
# RQ 2: and between the size classes?
# RQ 3: Is there a statistical difference between reef type between L0 vs L1? 
...
xmulti2 <- XNomial::xmulti(obs  = as.numeric(tab2_L1_nz),
...
# RQ 2 size class difference in reef preference at L1
...
# Post-hoc pairwise G-tests between size_class
...
# Generate CLD letters for each panel
cld_combined2 <- data.frame(stage = "L1", panel = "Combined", reef_type  = names(gtest_to_cld(pgtest2)), cld_letter = unname(gtest_to_cld(pgtest2)))

cld_large <- data.frame(stage = "L1", panel = "Large", reef_type  = names(gtest_to_cld(pgtest_large)), cld_letter = unname(gtest_to_cld(pgtest_large)))

cld_medium <- data.frame(stage = "L1", panel = "Medium", reef_type  = names(gtest_to_cld(pgtest_medium)), cld_letter = unname(gtest_to_cld(pgtest_medium)))

cld_small <- data.frame(stage = "L1", panel = "Small", reef_type  = names(gtest_to_cld(pgtest_small)),cld_letter = unname(gtest_to_cld(pgtest_small)))

cld_all2 <- bind_rows(cld_combined2, cld_large, cld_medium, cld_small)
