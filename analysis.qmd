...
write_out <- function(df, name) {
  path <- file.path(der_data_dir, name)
  suppressWarnings(unlink(path))
...
format_brms <- function(model, param, post_prob = NULL) {
  ...
  base <- paste0("($\\beta$ = ", b, ", 95% CI: ", low, " to ", high)
...
# Diagnostics
tryCatch(nuts_params(m_exp1_bayes) %>% filter(Parameter == "divergent__") %>% summarise(sum(Value)), error = function(e) invisible(NULL))
tryCatch(summary(m_exp1_bayes), error = function(e) invisible(NULL))

# Posterior probability of sex effect
p_post_sexM1 <- tryCatch(mean(as_draws_df(m_exp1_bayes)$b_muWoodLog_sexM > 0), error = function(e) NA_real_)
...
# Diagnostics
tryCatch(nuts_params(m_exp2_bayes) %>% filter(Parameter == "divergent__") %>% summarise(sum(Value)), error = function(e) invisible(NULL))
tryCatch(summary(m_exp2_bayes), error = function(e) invisible(NULL))
tryCatch(fixef(m_exp2_bayes), error = function(e) invisible(NULL))

# Posterior probability that medium preference for FlatStone is negative
p_post_flatM <- tryCatch(mean(as_draws_df(m_exp2_bayes)$b_muFlatStone_size_classM < 0), error = function(e) NA_real_)
...
# Diagnostics
tryCatch(nuts_params(m_exp3_bayes) %>% filter(Parameter == "divergent__") %>% summarise(sum(Value)), error = function(e) invisible(NULL))
tryCatch(summary(m_exp3_bayes), error = function(e) invisible(NULL))
tryCatch(fixef(m_exp3_bayes), error = function(e) invisible(NULL))

# Posterior probability size class effects are negative (prefer FlatStone)
p_post3_M <- tryCatch(mean(as_draws_df(m_exp3_bayes)$b_size_classM < 0), error = function(e) NA_real_)
p_post3_L <- tryCatch(mean(as_draws_df(m_exp3_bayes)$b_size_classL < 0), error = function(e) NA_real_)
...
# Diagnostics
tryCatch(nuts_params(m_exp4_bayes) %>% filter(Parameter == "divergent__") %>% summarise(sum(Value)), error = function(e) invisible(NULL))
tryCatch(summary(m_exp4_bayes), error = function(e) invisible(NULL))

# Posterior probability sex effect (negative = prefer ConcreteBrick)
p_post_sexM <- tryCatch(mean(as_draws_df(m_exp4_bayes)$b_sexM < 0), error = function(e) NA_real_)
