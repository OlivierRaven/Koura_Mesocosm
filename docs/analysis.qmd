...
REEF_LEVELS4 <- c("BrickConcrete","BrickWood","TankWall")
...
pct_wall1 <- round(sum(df1$reef_type == "TankWall" & df1$stage == "L1") / sum(df1$stage == "L1") * 100)
...
exp1_reef_letters <- exp1_reef_combined %>%
  relevel_reef() %>%
  filter(stage == "L1") %>%
  left_join(cld_all1, by = c("stage", "panel", "reef_type"))

exp1_p_combined <- exp1_reef_combined %>%
  relevel_reef() %>%
...
exp1_reef_letters_sex <- exp1_reef_bysex %>%
  left_join(cld_all1, by = c("stage", "panel", "reef_type"))

exp1_p_sex <- exp1_reef_bysex %>%
  relevel_reef() %>%
  ggplot(aes(reef_type, prop, fill = panel)) +
  ...
  geom_text(data = exp1_reef_letters_sex,  aes(y = prop + 0.04, label = cld_letter), position = position_dodge(0.9), size = 3.5, fontface = "bold") +
  scale_fill_manual(values = c(Female = "white", Male = "black"), name = "Sex") +
  labs(x = "Reef type", y = NULL) 

exp1_reef_p <- exp1_p_combined + exp1_p_sex 
...
pct_wall2 <- round(sum(df2$reef_type == "TankWall" & df2$stage == "L1") /
...
ce2_p <- pp_summary2 %>%
  ...
  ggplot(aes(reef_type, mean_prob, shape = size_class, group = size_class)) +
  ...
  labs(x = "Reef type", y = "Predicted probability (95% CI)", shape = "Size class", tag = "B")
...
pct_wall3 <- round(sum(df3$reef_type == "TankWall" & df3$stage == "L1") / sum(df3$stage == "L1") * 100)
...
ann_wilcox3 <- data.frame(
  ...
  mutate(y = prop + 0.04, label = case_when( reef_type == "FlatStone" & label != "ns" ~ paste0(label, "↑"), reef_type %in% c("WoodSplit") & label != "ns" ~ paste0(label, "↓"), TRUE ~ label))
...
ce3_p <- pp_summary3 %>%
  filter(reef_type != "NA", reef_type != "TankWall") %>%
  relevel_reef3() %>%
  mutate(size_class = factor(size_class, levels = c("S", "M", "L"))) %>%
  ggplot(aes(reef_type, mean_prob, shape = size_class, group = size_class)) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.15, position = position_dodge(0.5)) +
...
pct_wall4 <- round(sum(df4$reef_type == "TankWall" & df4$stage == "L1") / sum(df4$stage == "L1") * 100)
...
wilcox_bart  <- wilcox.test(df4_grp$prop[df4_grp$reef_type == "BrickConcrete"],  mu = .5, exact = FALSE)
...
ba_props <- df4_grp %>% filter(reef_type == "BrickConcrete") %>% pull(prop)
...
pct_bart4  <- round(mean(df4_grp$prop[df4_grp$reef_type == "BrickConcrete"]) * 100)
pct_bwood4 <- round(mean(df4_grp$prop[df4_grp$reef_type == "BrickWood"]) * 100)
...
ann_wilcox4 <- data.frame(reef_type = c("BrickConcrete", "BrickWood"),label = c(sig_label(p_wilcox4_adjusted["p_wilcox4_bart"]), sig_label(p_wilcox4_adjusted["p_wilcox4_bwood"]))) %>%
  mutate(reef_type = factor(reef_type, levels = REEF_LEVELS4)) %>%
  left_join(exp4_combined %>% select(reef_type, prop), by = "reef_type") %>%
  mutate(y = prop + 0.04, label = case_when( reef_type %in% c("BrickConcrete", "BrickWood") & label != "ns" ~ paste0(label, "↑"), TRUE ~ label))
...
ce4_p <- pp_summary4 %>%
  filter(reef_type != "TankWall") %>%
  relevel_reef4() %>%
  ggplot(aes(reef_type, mean_prob, shape = sex, group = sex)) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.15, position = position_dodge(0.5)) +
  geom_point(size = 3, position = position_dodge(0.5)) +
  scale_shape_manual(values = sex_shape_map, labels = c(F = "Female", M = "Male")) +
  labs(x = "Reef type", y = "Predicted probability (95% CI)", shape = "Sex", tag = "B")
