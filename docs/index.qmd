...
# Results
## Exp 1: Single kōura
Individual kōura showed a strong preference for WoodLog, selecting it significantly more often than SingleStone, FlatStone, and StonePile (pairwise G-test, Bonferroni adjusted: *p* = `r format_pval(p_single_vs_wood1)`, `r format_pval(p_flat_vs_wood1)`, `r format_pval(p_pile_vs_wood1)`, respectively; overall multinomial test: *p* = `r format_pval(p_xmulti1)`; @fig-exp1). No significant differences were found among the stone reef types. Overall, WoodLog was selected by `r pct_wood_overall`% of kōura at L1 (males: `r pct_wood_male`%; females: `r pct_wood_female`%), and no kōura were found at TankWall, indicating all individuals actively selected a reef structure. Reef preference did not differ significantly between sexes (Fisher's exact test: *p* = `r format_pval(p_fisher1)`). The symmetry test indicated that reef type use shifted between L0 and L1 (*p* = `r format_pval(p_symtest1)`), suggesting kōura actively relocated to preferred reef types overnight rather than remaining at their initial location.

## Exp 2: Group kōura
When housed in mixed-size groups, kōura showed a significant overall preference for stone-based reefs (@fig-exp2). FlatStone was used significantly more (*p* = `r format_pval(p_wilcox2_flat)`), while SingleStone (*p* = `r format_pval(p_wilcox2_single)`) and WoodLog (*p* = `r format_pval(p_wilcox2_wood)`) were significantly avoided. StonePile did not differ from random expectation (*p* = `r format_pval(p_wilcox2_pile)`), and did not differ significantly from FlatStone (*p* = `r format_pval(p_fs_vs_sp)`). No kōura were found at TankWall, indicating active reef selection across the group. Size classes had limited but credible effect on reef preference. Medium sized kōura showed a weaker preference for FlatStone over SingleStone compared to small kōura `r format_brms(m_exp2_bayes, "muFlatStone_size_classM", p_post_flatM)`, while no other size class effect was credible. Overall, FlatStone remained the most selected reef type across all size classes, but its strength of preference declined from small (posterior probability of top choice = `r pct_top_S`%) to medium (`r pct_top_M`%), and with large kōura showing similar preference between FlatStone and StonePile (`r pct_flat_L`% vs `r pct_pile_L`%). Reef type use shifted directionally between L0 and L1 (Stuart-Maxwell test: *p* = `r format_pval(p_smtest2)`), indicating that kōura actively relocated to preferred reef types overnight. 

## Exp 3: Wood vs Stone
Kōura consistently selected FlatStone over WoodSplit, with the preference growing with body size (@fig-exp3). At the group level, FlatStone was used significantly more than expected (*p* = `r format_pval(p_wilcox3_flat)`), while WoodSplit was significantly avoided (*p* = `r format_pval(p_wilcox3_split)`). Only `r pct_wall3`% of kōura were found at TankWall, suggesting almost all kōura selected one of the two reef types. The Bayesian model indicated that preference for FlatStone strengthened with body size, with the posterior probability of FlatStone being the top-ranked reef increasing from small (`r pct_top3_S`%) to medium (`r pct_top3_M`%), to large kōura (`r pct_top3_L`%). Large kōura showed a particularly strong and credible preference for FlatStone over WoodSplit `r format_brms(m_exp3_bayes, "muWoodSplit_size_classL", mean(as_draws_df(m_exp3_bayes)$b_muWoodSplit_size_classL < 0))`, while medium kōura did not differ from small kōura. Reef type use shifted directionally between L0 and L1 (Stuart-Maxwell test: *p* = `r format_pval(p_smtest3)`), with kōura relocating toward FlatStone overnight.

## Exp 4: Bricks
Kōura used both brick types, showing no significant preference between BrickArtificial and BrickWood (*p* = `r format_pval(p_bw_vs_ba)`; @fig-exp4), with mean group proportions of `r pct_bart4`% and `r pct_bwood4`% respectively. Neither brick type differed significantly from equal distribution (*p* = `r format_pval(p_wilcox4_bart)`). `r pct_wall4`% of kōura were found at TankWall, which is higher than in any previous experiment. This suggests that brick structures were rejected by a greater proportion of individuals than natural reef materials. Sex had no credible effect on preference between brick types `r format_brms(m_exp4_bayes, "muBrickWood_sexM", p_post_sexM)`, indicating that concrete and wooden perforated brick structures of identical dimensions were both suitable refuge for male and female kōura.

# Discussion

## Structure matters more than material
Across four experimtnes, refuge configuration and structural stability consistently predicted kōura habitat selection more strongly than material composition alone. Experiment 4 shows that when kōura were presented with similar refuge sizes in concrete bricks and wooden bricks no specific preference for material was suggested. In experiment 3 FlatStone and WoodSplits were tested against each other and showed a difference in reef preference likely due to the stability of the stones compared to the wood splits. The WoodSplits were sinking but created less structurally stable refuge places in the sandy substrate than the FlatStone reef [O.V. Raven, pers. obs.]. Experiment 2 showed that when a group of kōura is presented with multiple reef types the reefs that provide the most crevice complexity drive group-level preference. This was the case for FlatStone followed by StonePile for harbouring the most kōura. In experiment 1 the preference was different as individual kōura preferred WoodLog as refuge, probably as single kōura searched the refuge that would give most protection [@Jurcak2016], and the drilled holes in the WoodLog offer most protection similar to what has been found for PVC tubes [@Zhao2024]. The directional shift in reef type use between L0 and L1 confirmed across all experiments that kōura actively relocated overnight rather than remaining at their initial location, supporting the interpretation that next-day locations reflect genuine refuge preference rather than chance displacement. The preference shift from WoodLog in single to stone reefs in group experiments was unexpected. It might be explained by size mismatch between fixed hole dimensions and the range of body sizes present in each group, while the large surface area of FlatStone allowied multiple individuals to shelter simultaneously and reducing competitive displacement. Another effect could play a role as in the first round of experiment 2 WoodLog was most selected reef (17 out of 36 times), it could be that kōura learned that the WoodLog is not ideal and moved to other reefs. This learning was possible as the groups of kōura in experiment 2 were reused all five rounds, while the single kōura in experiment 1 were only used once. The reuse of groups across rounds was necessary given the limited availability of kōura and ethical constraints on collection numbers. The number of independent groups was low across experiments (six in experiments 2 and 3, four in experiment 4), constraining the statistical power for detecting group-level effects [@Hurlbert1984]. Despite these constraints, consistent patterns in size-dependent preference were evident across experiments. 

## Body size and competitive dynamics
Size class consistently influenced refuge preference across experiments, but the pattern differed by context. In experiment 3, larger kōura showed a stronger preference for FlatStone over WoodSplit than smaller individuals. This may reflect larger kōura being more capable of asserting dominance over preferred stable refugia through competitive displacement, consistent with size-based dominance hierarchy documented in kōura populations [@Stewart2011; @Ranta1993; @Figler2005]. In experiment 2, where four reef types were available, small kōura showed the strongest preference for FlatStone while large kōura distributed more evenly between FlatStone and StonePile, suggesting that when multiple stone reefs are available larger individuals are less selective between stone configurations, likely because both provide suitable sized crevices for larger bodies. Small kōura may have particularly benefited from FlatStone's large continuous surface area, which provides the most accessible refuge spaces also for smaller body dimensions without creating competition with larger individuals.

## Restoration implications
The finding that structural configuration and stability drive refuge preference more strongly than material composition has broad implications for artificial reef design beyond kōura restoration. The availability of appropriate size and structurally stable crevices appears to be the most important driver for freshwater artificial reef use by crayfish [@Huolila1997; @Streissl2002]. Also in marine environments these principles may be applicable to other refuge-dependent species including lobsters and reef fish [@Spanier1994; @Hylkema2020]. Natural tree reef structures attracted crustaceans at tenfold higher abundances than bare substrate controls within six months of deployment [@Dickson2023]. Other important factors is that artificial reefs need to provide protection of predators, and there need to be access to food in the vicinity. When building artificial reefs natural materials should be favoured as materials that are locally sourced will have minimal impact on the ecosystem [@Wehi2017; @Cooke2023]. Natural materials have been shown to outperform plastic structures for freshwater fish habitat, with wood structures attracting more fish than plastic alternatives [@Magnelia2008]. in 

For kōura restoration in lake environments, stone pile configurations are recommended over flat arrangements. Pile structures provide a range of crevice sizes accommodating multiple size classes simultaneously and are less likely to be gradual buried in soft substrates than flat configurations. For in-lake deployment, cobble substrate of 100–300 mm is recommended as kōura are positively associated with coarser substrate in this size range [@Olsson2006; @Johnsen2008; @Jowett2008; @Kusabs2015]. Providing enclosed crevice spaces within reef structures is important in lakes were koura are preyed by invasive species such as catfish [@KusabsInrevision; @RavenInreview], as reef quality directly influences predation survival[@Zhao2024]. While these mesocosm findings provide clear design guidance, field validation in lake environments remains an essential next step, as in-lake conditions including water movement, natural substrate, and predator pressure may modify kōura preferences in ways not captured under controlled conditions.

## Conclusion
This study demonstrates that refuge configuration and structural stability are more important determints of kōura habitat preference than material composition alone. When structural properties differd stone based refugia had stronger preference this was not shown when dimantions and structure were kept constant. individual kōura preferd enclosed wood log structures while in groups kōura preferred 
